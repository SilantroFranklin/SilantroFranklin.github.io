...
updateAndReloadServiceWorker = (data) => {
    ...
    console.log("Version: " + data + " (new) force reloading service worker");
    // delete the cache
    caches.delete('zeitman-tracker').then(() => {
...
if (window.matchMedia('(display-mode: standalone)').matches){
    ...
    //alert("Please switch to the app version, see 'Get the App!'")
    if (FORCE_APP_MODE){
        window.location = '/force_get_app'
    }
} else {
    document.getElementsByClassName('gettheapp')[0].style.display = 'none';
}
